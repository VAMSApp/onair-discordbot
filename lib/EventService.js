...
class EventServiceClass {
    ...
    publish(channel, data) {
        ...
        Logger.info(`EventService::${channel} publishing message '${data}' to channel`)
    ...
    async subscribe(channel, client) {
        ...
        return new Promise((resolve, reject) => {
            ...
            self.Subscriber.subscribe(channel, (err, count) => {
                if (err) {
                    Logger.error(`⚠️ EventService::${channel} error subscribing: ${err}`);
                    return reject(err);
                }

                Logger.debug(`✅ EventService::${channel} subscribed`);
...
module.exports = EventServiceClass;