...
class OnAir {
    ...
    constructor() {
        ...
        if (this.Config.VAEvents.refreshOnStartup) {
    ...
    refreshVADetails() {
        return new Promise(async (resolve, reject) => {
            ...
            const vaDetails = await this.refreshVADetails();
            if (!vaDetails) return reject('no va details found');

            this.VA = vaDetails;

            return resolve(vaDetails);
        });
    }
    ...
    async refreshVANotifications() {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            if (x.length > 0) {
                ...
                if (!self.VA) return reject('No VA found');
