...
export function buildMembersList (x) {
    if (!x) return;
    let t = new Table;
...
export function buildFlightsList (x) {
    if (!x) return;
    let t = new Table;
    ...
    x.forEach(function (f) {
        ...
        if (f.StartTime && !arrivalIntendedAirport) {
            status = '🛫 In Progress'
        } else if (f.StartTime) {
            status = '🟢 Started'
        }  
...
export function buildJobsList (x) {
    if (!x) return;
    let t = new Table;
    ...
    x.forEach(function (j) {
        ...
        if (j.Cargos.length > 0) {
            t.cell('Cargos', `${j.Cargos.length} leg${(j.Cargos.length > 1) ? 's' : ''}`)
            t.newRow()
            ...
            j.Cargos.forEach(function (c) {
                ...
                t.cell('Type')
                t.cell('Cargos', c.CargoType.Name)
                t.cell('Status', status)
...
export function buildFleetList (x) {
    if (!x) return;
